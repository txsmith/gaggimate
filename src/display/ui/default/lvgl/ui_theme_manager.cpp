...
_ui_local_style_property_setting_t *_ui_local_style_property_setting_create(_ui_local_style_t *local_style_p, lv_obj_t *object_p,
    ...
    if (style_property_setting_p != NULL) { // checking might be unnecessary if count is properly initialized to 0
        if (local_style_p->garbage_collector_couter > 0)
            --local_style_p->garbage_collector_couter; // avoid calling garbage collector every time
     ...
     LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) // Not recommended inside explicit garbage collector.
                    if (style_property_setting_p->validcheck_counter > 0)
                        ...
                        if (lv_obj_is_valid(style_property_setting_p->object_p))
                            style_property_setting_p->validcheck_counter = LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE;
                        else
...
#if (defined(LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) &&                                                                      \
     LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) // for slow hardware, adding this and setting to 5..10 in lv_conf.h might make it
                                                 // faster
                    if (style_property_setting_p->validcheck_counter > 0)
                        --style_property_setting_p->validcheck_counter; // avoids calling complex lv_obj_is_valid() many times
                    else {                                              // check objects with expired positive vaidity check
